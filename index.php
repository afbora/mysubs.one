...
if (isset($_SESSION['loggedin']) === false || $_SESSION['loggedin'] !== true) {
    return include_once 'landing.php';
}
        ...
        <div class="top-actions">
            <div class="search">
                <input type="text" autocomplete="off" name="search" id="search"
                       placeholder="<?= translate('search', $i18n) ?>"
            ...
            <div class="filtermenu on-dashboard">
                ...
                <div class="filtermenu-content">
                    ...
                    if (count($members) > 1) {
                        ...
                        <div class="filtermenu-submenu">
                            <div class="filter-title"
                                 onClick="toggleSubMenu('member')"><?= translate("member", $i18n) ?></div>
                            <div class="filtermenu-submenu-content" id="filter-member">
                                ...
                                foreach ($members as $member) {
                                    ...
                                    ?>
                                    <div class="filter-item <?= $selectedClass ?>"
                                         data-memberid="<?= $member['id'] ?>"><?= $member['name'] ?>
                                    </div>
                                    <?php
                                }
                                ?>
                    ...
                    if (count($categories) > 1) {
                        ...
                        <div class="filtermenu-submenu">
                            <div class="filter-title"
                                 onClick="toggleSubMenu('category')"><?= translate("category", $i18n) ?></div>
                    ...
                    if (count($payment_methods) > 1) {
                        ...
                        <div class="filtermenu-submenu">
                            <div class="filter-title"
                                 onClick="toggleSubMenu('payment')"><?= translate("payment_method", $i18n) ?></div>
                    ...
                    if (!isset($settings['hideDisabledSubscriptions']) || $settings['hideDisabledSubscriptions'] !== 'true') {
                        ...
                        <div class="filtermenu-submenu">
                            <div class="filter-title"
                                 onClick="toggleSubMenu('state')"><?= translate("state", $i18n) ?></div>
                            <div class="filtermenu-submenu-content" id="filter-state">
                                <div class="filter-item capitalize"
                                     data-state="0"><?= translate("enabled", $i18n) ?></div>
                                <div class="filter-item capitalize"
                                     data-state="1"><?= translate("disabled", $i18n) ?></div>
                            </div>
                        </div>
                        <?php
                    }
            ...
            <div class="sort-container">
                ...
                <div class="sort-options" id="sort-options">
                    <ul>
                        <li <?= $sortOrder == "name" ? 'class="selected"' : "" ?> onClick="setSortOption('name')"
                                                                                  id="sort-name">
                            <?= translate('name', $i18n) ?>
                        </li>
                        <li <?= $sortOrder == "id" ? 'class="selected"' : "" ?> onClick="setSortOption('id')"
                                                                                id="sort-id">
                            <?= translate('last_added', $i18n) ?>
                        </li>
                        <li <?= $sortOrder == "price" ? 'class="selected"' : "" ?> onClick="setSortOption('price')"
                                                                                   id="sort-price">
                            <?= translate('price', $i18n) ?>
                        </li>
                        <li <?= $sortOrder == "next_payment" ? 'class="selected"' : "" ?>
                                onClick="setSortOption('next_payment')"
                                id="sort-next_payment"><?= translate('next_payment', $i18n) ?></li>
                        <li <?= $sortOrder == "payer_user_id" ? 'class="selected"' : "" ?>
                                onClick="setSortOption('payer_user_id')"
                                id="sort-payer_user_id"><?= translate('member', $i18n) ?></li>
                        <li <?= $sortOrder == "category_id" ? 'class="selected"' : "" ?>
                                onClick="setSortOption('category_id')"
                        ...
                        if (!isset($settings['hideDisabledSubscriptions']) || $settings['hideDisabledSubscriptions'] !== 'true') {
                            ?>
                            <li <?= $sortOrder == "inactive" ? 'class="selected"' : "" ?>
                                    onClick="setSortOption('inactive')"
                                    id="sort-inactive"><?= translate('state', $i18n) ?></li>
                            <?php
                        }
                        ?>
                        <li <?= $sortOrder == "alphanumeric" ? 'class="selected"' : "" ?>
                                onClick="setSortOption('alphanumeric')"
...
<section class="subscription-form" id="subscription-form">
    ...
    <form action="endpoints/subscription/add.php" method="post" id="subs-form">
        ...
        <div class="form-group-inline">
            ...
            <div id="logo-search-button" class="image-button medium disabled"
                 title="<?= translate('search_logo', $i18n) ?>"
                 onClick="searchLogo()">
        ...
        <div class="buttons">
            <input type="button" value="<?= translate('delete', $i18n) ?>" class="warning-button left thin"
                   id="deletesub"
                   style="display: none">
